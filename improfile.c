...
{
    if (CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR_NOT_IMG) + CLI_checkarg(3, CLIARG_FLOAT) +
            CLI_checkarg(4, CLIARG_FLOAT) + CLI_checkarg(5, CLIARG_FLOAT) + CLI_checkarg(6, CLIARG_LONG) ==
        0)
    {
        profile(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string, data.cmdargtoken[3].val.numf,
                data.cmdargtoken[4].val.numf, data.cmdargtoken[5].val.numf, data.cmdargtoken[6].val.numl);
...
errno_t improfile_addCLIcmd()
{
    ...
    RegisterCLIcommand("profile", __FILE__, info_profile_cli, "radial profile",
                       ...
                       "int profile(const char *ID_name, const char *outfile, double xcenter, double ycenter, double "
                       "step, long nb_step)");
...
errno_t profile2im(const char *profile_name, long nbpoints, unsigned long size, double xcenter, double ycenter,
                   double radius, const char *out)
{
    DEBUG_TRACE_FSTART();
    ...
    for (unsigned long ii = 0; ii < size; ii++)
        ...
        {
            r = sqrt((1.0 * ii - xcenter) * (1.0 * ii - xcenter) + (1.0 * jj - ycenter) * (1.0 * jj - ycenter)) /
                radius;
            i = (long)(r * nbpoints);
