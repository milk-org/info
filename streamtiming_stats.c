...
errno_t info_image_streamtiming_stats_disp(double *tdiffvarray, long NBsamples, float *percarray, long *percNarray,
                                           long NBperccnt, long percMedianIndex, long cntdiff, long part, long NBpart,
                                           double tdiffvmax, long tdiffcntmax);
...
errno_t info_image_streamtiming_stats(const char *ID_name, int sem, long NBsamples, long part, long NBpart)
{
    ...
    info_image_streamtiming_stats_disp(tdiffvarray, NBsamples, percarray, percNarray, NBperccnt, percMedianIndex,
                                       cntdiff, part, NBpart, tdiffvmax, tdiffcntmax);
    ...
    for (perccnt = 0; perccnt < NBperccnt; perccnt++)
    {
        if (perccnt == percMedianIndex)
        {
            ...
            printw("%6.3f% \%  %6.3f% \%  [%10ld] [%10ld]    %10.3f us\n", 100.0 * percarray[perccnt],
                   100.0 * (1.0 - percarray[perccnt]), percNarray[perccnt], NBsamples - percNarray[perccnt],
