...
errno_t cubeMatchMatrix_addCLIcmd()
{
    ...
    RegisterCLIcommand("cubeslmatch", __FILE__, info_cubeMatchMatrix_cli, "compute sqsum differences between slices",
                       "<imagecube> <output file>", "cubeslmatch incube outim",
...
imageID info_cubeMatchMatrix(const char *IDin_name, const char *IDout_name)
{
    ...
    for (kk1 = 0; kk1 < zsize; kk1++)
        for (kk2 = kk1 + 1; kk2 < zsize; kk2++)
        {
            ...
            if (((double)data.image[IDout].array.F[kk2 * zsize + kk1] > 1.0) && (kk2 - kk1 > kdiffmin) &&
                (kk2 - kk1 < kdiffmax))
            {
