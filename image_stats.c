...
{
    RegisterCLIcommand("imstats", __FILE__, info_image_stats_cli, "image stats", "<image>", "imgstats im1",
                       "int info_image_stats(const char *ID_name, \"\")");

    RegisterCLIcommand("imstatsf", __FILE__, info_image_statsf_cli, "image stats with file output", "<image>",
...
errno_t info_image_stats(const char *ID_name, const char *options)
{
    ...
    if (ID != -1)
    {
        ...
        printf("write = %d   cnt0 = %ld   cnt1 = %ld\n", data.image[ID].md[0].write, data.image[ID].md[0].cnt0,
